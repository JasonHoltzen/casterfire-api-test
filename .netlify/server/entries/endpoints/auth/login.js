...
async function post({ body }) {
  if (!body) {
    throw (0, import_erroh_c062e309.b)();
  }
  try {
    const { email, password } = body;
    await (0, import_db_8890404f.c)();
    let user = await import_User_a98eda05.U.findOne({ email }).lean().clone();
    if (!user)
    ...
    return (0, import_erroh_c062e309.u)("None...shall...pass...without the correct email and password.");
  } catch (err) {
    console.log(err);
